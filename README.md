# SigSmith 🔨🔥

**Forge Your Professional Email Signature**

SigSmith is a minimal, professional email signature generator designed with a modern glassmorphism theme. It allows users to create high-quality, HTML-compatible email signatures that work seamlessly across major email clients like Gmail, macOS Mail, and iOS Mail.

## ✨ Features

- **Live Demo**: [https://sigsmith.vercel.app/](https://sigsmith.vercel.app/)
- **Security First**: Built-in XSS protection and input sanitization.
- **Performance Optimized**: Memoized components and optimized handlers for a smooth experience.
- **Responsive Design**: Fully functional on mobile and desktop devices.
- **Live Preview**: Real-time signature updates as you type.
- **Rich Text Copy**: One-click "Copy Signature" that preserves formatting and HTML structure.
- **Theme**: Professional dark aesthetic with glassmorphism and subtle animations.
- **Logo Upload**: Personalize your signature with your own company logo.
- **Import Guides**: Integrated instructions for setting up your signature in various email clients.

## 🛠️ Tech Stack

- **Framework**: [Next.js 15+](https://nextjs.org/) (App Router)
- **Styling**: [Tailwind CSS 4](https://tailwindcss.com/)
- **Animations**: [Motion](https://motion.dev/) (Framer Motion)
- **Icons**: [Lucide React](https://lucide.dev/)
- **Language**: TypeScript

## 🚀 Getting Started

1. **Install Dependencies**:
   ```bash
   npm install
   ```

2. **Run Development Server**:
   ```bash
   npm run dev
   ```

3. **Build for Production**:
   ```bash
   npm run build
   ```

## 📜 License

MIT
